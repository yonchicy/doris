...
suite("test_rollup_partition_mtmv") {
    ...
    """
    def listMonthPartitions = sql """show partitions from ${mvName}"""
    logger.info("listMonthPartitions: " + listMonthPartitions.toString())
    assertEquals(2, listMonthPartitions.size())
    assertTrue(listMonthPartitions.toString().contains("2020-01-01"))
    assertTrue(listMonthPartitions.toString().contains("2020-02-01"))
    sql """drop materialized view if exists ${mvName};"""
